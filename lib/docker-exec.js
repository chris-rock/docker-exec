...
DockerExec.prototype.pullContainer = function (image) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        // we use docker run, because the dockerode "docker.pull(image, function (err, stream)"
        // return too early
        docker.run(image, ['bash', '-c', 'uname -a'], process.stdout, function () {
            // do nothing
        })
        // wait until the image is pulled and the container is running
        .on('container', function (container) {

            // container should stop immediately, so let's continue
            resolve();
