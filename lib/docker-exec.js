 ...
 */
DockerExec.prototype.startContainer = function (container) {
    debug('start a new container');

    var self = this;

    return new Promise(function (resolve, reject) {
        container.start({}, function handler(err, data) {
            if (err) {
                reject(err);
            } else {
...
DockerExec.prototype.pullContainer = function () {
    debug('download operating system');

    var docker = this.docker;

    return new Promise(function (resolve, reject) {
        docker.pull('ubuntu:14.04', function (err, stream) {
            if(err) {
                reject(err);
            } else {
                resolve();
 ...
 */
DockerExec.prototype.start = function () {
    debug('start a new container');
    ...
    return new Promise(function (resolve, reject) {
        console.log('prepare new container');

        // attach a volume to docker to copy the files afterwards
        self.pullContainer().then(function() {
            ...
            self.stream = stream;
            return self.startContainer(self.container);
        }).then(function () {
...
DockerExec.prototype.run = function (command) {
    ...
    return new Promise(function (resolve) {
        ...
        // wait for exit from process
        self.stream.on('data', function (data) {
            // wait for exit event
            // make abort condition with var exit = 'task ' + id;
            var match = data.toString('utf8').match(/((;echo\s)?exit)/i);
            if (match && match.length > 0 && match[0] === 'exit') {
                // TODO use buffer of 10 events
                // TODO make more robust
                // console.log('detected exit' + JSON.stringify(match));
                resolve();
            }
        });
    });
